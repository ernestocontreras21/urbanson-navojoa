    ...
    function agregarMensaje(texto, tipo) {
        ...
        if (tipo === 'bot') {
            texto = texto
                ...
                .replace(/\[PARADA[\s\S]*?:\s*(\d+)\s*:\s*([^\]]+?)\s*\]/g, (_, id, nombre) => {
                    return `<button onclick="window._chatbotSeleccionarParada('${id}','${nombre.trim()}')" style="display:block;margin:4px 0;padding:6px 12px;border-radius:20px;border:1.5px solid #7a1028;background:white;color:#7a1028;cursor:pointer;font-family:'Outfit',sans-serif;font-weight:600;font-size:12px;text-align:left;width:100%;">📍 ${nombre.trim()}</button>`;
                })
                .replace(/\n/g, '<br>');
        }
